...
unique_date_lable1 = date_obj.day
...
def process_day_data_downloaded_maharajgunj(i, algorithms, date_lable1):
    dataframeslist = []
    for algo in algorithms:
        df = pd.read_csv(f"D:\\ML Predictions\\AirPollutionUI\\air-pollution-ml\\flask-server\\Backup_downloads\\Maharajgunj{i}{algo}{date_lable1}d.csv")
...
def process_day_data_downloaded_phora(i, algorithms, date_lable_1):
    dataframeslist = []
    for algo in algorithms:
        df = pd.read_csv(f"D:\\ML Predictions\\AirPollutionUI\\air-pollution-ml\\flask-server\\Backup_downloads\\Phora{i}{algo}{date_lable_1}d.csv")
        # Ensure object columns are cast to strings
        for col in df.select_dtypes(include=['object']).columns:
            df[col] = df[col].astype(str)
        dataframeslist.append(df)
...
#For Maharajgunj
for i in range (7):
    df1 = process_day_data_downloaded_maharajgunj(i+1, algorithms, unique_date_lable1)
...
#For Phora Durbar
for i in range (7):
    df1 = process_day_data_downloaded_phora(i+1, algorithms, unique_date_lable1)
...
def max_value_times_daily_maharajgunj(day):
    ...
    df = process_day_data_downloaded_maharajgunj(day, algorithms, unique_date_lable1)
    highest_aqi_hour = []
    for j in range (3):
...
st.divider()
st.subheader("ii. Display the Status of Air Quality and generate alert/recommendation messages for Phora Durbar")

#returns a list for highest_aqi_hour for three algorithms respectively
def max_value_hour_daily_phora(day):
    
    df = process_day_data_downloaded_phora(day, algorithms, unique_date_lable1)
    highest_aqi_hour = []
    for j in range (3):
        highest = df[j].loc[df[j][f'AQI_{algorithms[j]}'].idxmax()]
        hour = highest['Hour']
...
if submit_button1:
    if recipient1 and subject1 and html_message_02:
        result1 = send_email_with_attachment(recipient1, subject1, html_message_02, process_day_data_downloaded_maharajgunj(option1, algorithms, unique_date_lable1))
...
if submit_button1:
    if recipient1 and subject1 and html_message_04:
        result1 = send_email_with_attachment(recipient1, subject1, html_message_04, process_day_data_downloaded_phora(option1, algorithms, unique_date_lable1))
        st.success(result1) if "successfully" in result1 else st.error(result1)
    else:
        st.error("Please fill in all fields.")
